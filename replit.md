...
- **`resolverLojaPorRegiao` (SAFE PROD)**: usa SOMENTE `id`, `nomeFantasia`, `endereco`, `razaoSocial` — NUNCA usa `regiao`, `bairrosAtendidos`, `cidade`, `uf` em where. Busca por termos livres (regiaoCliente, bairroCliente, cidadeCliente) em OR contra esses 3 campos.
- **`prisma.lead.create` com fallback**: tenta `dadosExtendidos` (campos novos) primeiro; se falhar, retenta com `dadosBase` (campos originais que sempre existem em prod). Nunca quebra o endpoint por schema desatualizado.
